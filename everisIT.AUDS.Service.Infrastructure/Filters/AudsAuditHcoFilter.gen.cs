...
{
    public partial class AudsAuditHcoFilter : Interfaces.IAudsAuditHcoFilter
    {
        /// <summary>
        ...
        public IQueryable<Models.AudsAuditHco> Filtrate(IQueryable<Models.AudsAuditHco> entityModel)
        {
            if (CodeStatus != null && CodeStatus.HasValue)
                entityModel = entityModel.Where(entityModel => entityModel.CodeStatus.Equals(CodeStatus));

            if (AuditHcoId != null)
                entityModel = entityModel.Where(model => model.AuditHcoId == AuditHcoId);
